...
def consolidate(lang,month_list,year):
	source0, target0, source1, target1 = [], [], [], []
	lang_dict={'hi':'hindi','bn':'bengali'}
	...
	print(f"English {len_e}, {lang_dict[lang]} {len_b}, En-{lang} Pair {len_eb}")
	print(f"English {len_ae}, {lang_dict[lang]} {len_ab}, En-{lang} Pair {len_aeb}")
	...
	print(len(match),len(almost_match))
	match.to_csv(
	    os.path.join(out_dir, f"Total_dwnews_{lang}-En_{year}_Aligner_Match.csv"),
	    encoding=config.CSV_FILE_ENCODING,
	    index=False,
	)
	match.to_csv(
	    os.path.join(out_dir, f"Total_dwnews_{lang}-En_{year}_Aligner_Match.tsv"),
	    encoding=config.CSV_FILE_ENCODING,
	    index=False,
	    sep = '\t'
	)
	almost_match.to_csv(
	    os.path.join(out_dir, f"Total_dwnews_{lang}-En_{year}_Aligner_Almost_Match.csv"),
	    encoding=config.CSV_FILE_ENCODING,
	    index=False,
	)
...
def main():
	...
	if args.month_list == "all":
		g_dict[lang].capitalize()+' Sentences'
	match = pd.DataFrame({'English Sentences': target0, head2: source0}) 
	almost_match = pd.DataFrame({'English Sentences': target1, head2: source1})
	len_e = match.duplicated(subset=['English Sentences']).sum()
	len_b = match.duplicated(subset=[head2]).sum()
