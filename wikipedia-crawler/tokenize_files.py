...
import pandas as pd
from tokenize_utils import tokenize_eng_file, tokenize_hi_file,tokenize_other_lang


lang = 'kn'

b_path = os.path.join(r'C:\Users\Dhanvi\Wikipedia_Scraping','Scraped_Files')
output_path = os.path.join(r'C:\Users\Dhanvi\Wikipedia_Scraping', 'Tokenized_Files-'+lang)
...
for f in os.listdir(b_path):
    if('-en.txt' in f and not os.path.exists(os.path.join(output_path,f))):
        id = f.split('-')[0]
        ...
        if(lang == 'hi'):
            ...
            sent_tgt = tokenize_other_lang(fintgt.read(),lang)
        finsrc.close()
        fintgt.close()
