...
def consolidate(lang,year,month_list):
	...
	lang_dict={'hi':'hindi','te':'telugu','kn':'kannada','ta':'tamil','ml':'malayalam','mr':'marathi','gu':'gujarati','pa':'punjabi'}
	...
	match.to_csv(
	    os.path.join(out_dir, f"Total_tribune_{lang}-En_{year}_Aligner_Match.csv"),
	    encoding=config.CSV_FILE_ENCODING,
	    index=False,
	)
	match.to_csv(
	    os.path.join(out_dir, f"Total_tribune_{lang}-En_{year}_Aligner_Match.tsv"),
	    encoding=config.CSV_FILE_ENCODING,
	    index=False,
	    sep = '\t'
	)
	almost_match.to_csv(
	    os.path.join(out_dir, f"Total_tribune_{lang}-En_{year}_Aligner_Almost_Match.csv"),
	    encoding=config.CSV_FILE_ENCODING,
	    index=False,
	)
