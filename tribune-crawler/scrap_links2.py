...
def retry_scraper(month,year,lang_code,inp_dir,save_dir,log,count_list):
	...
	for count in count_list:
		...
		try:
			...
			if markup_string.status_code == "404":
				print(count,"error---------",link)
				count+=1
				continue
			markup_string = markup_string.content
			soup = BeautifulSoup(markup_string, "lxml")
			head=[]
			try:
				head.extend(soup.findAll(["p","font"]))		#Headline
			except:
				print(count,"found-None",link)
			...
			with open(os.path.join(str(save_dir), f"TOI-{lang_code}-{month[:3]}-{year}-{count}.txt"), mode="w", encoding="utf-16") as file_w:
				...
				if os.stat(os.path.join(str(save_dir), f"TOI-{lang_code}-{month[:3]}-{year}-{count}.txt")).st_size == 0:
...
def scraper(month,year,lang_code,inp_dir,save_dir,count,log,retry):
	...
	for link in articles_data['Link'][count:]:
		...
		try:
			markup_string = requests.get(link, stream=True)
			scode=markup_string.status_code
			if markup_string.status_code == 404:
				print(count,"404---------",link)
				count+=1
				continue
			markup_string=markup_string.content
			soup = BeautifulSoup(markup_string, "lxml")
			...
			with open(os.path.join(str(save_dir), f"TOI-{lang_code}-{month[:3]}-{year}-{count}.txt"), mode="w", encoding="utf-16") as file_w:
				...
				if log : print(f"tribune-{lang_code}-{month[:3]}-{year}-{count}.txt")
				if os.stat(os.path.join(str(save_dir), f"TOI-{lang_code}-{month[:3]}-{year}-{count}.txt")).st_size == 0:
					print(count,"empty",link,scode,type(scode))
				count+=1
		except:
			print(count,"error---------",link,scode)
