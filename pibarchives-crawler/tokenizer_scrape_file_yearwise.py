...
from utilities import create_directory, write_sentence_list_to_file, api_sen_tokenizer_call
...
def tokenized_scrape_file(n_month,n_year,lang,out_dir):
    ...
    if lang == 'en':
        ...
        return None
    total_sen_pd = pd.DataFrame(columns=[language + '_sen'])
    ...
    fl_check = sorted([os.path.basename(i) for i in glob.glob(os.path.join(tokenize_loc, '*.txt'))])
    for k, fl in enumerate(fl_list[start_from:]):
        print(os.path.basename(fl))
        flname = tokenize_loc + '//tok_' + os.path.basename(fl)
        with open(fl, mode='r', encoding='utf-16') as file_n:
            para_val = [{'text': line.strip()} for line in file_n if len(line.strip().split()) > 2]
        if len(para_val) > 500:
            sen = []
        ...
        dump_1 = pd.DataFrame(sen, columns=['sen']).drop_duplicates().loc[:, 'sen'].values.tolist()
        sen = dump_1
...
def total_sen_csv_from_tokenized_file(n_month,n_year,lang,out_dir):
    ...
    for k, fl in enumerate(fl_list):
        with open(fl, mode='r', encoding='utf-16') as file_n:
            for line in file_n:
                total_pd = total_pd.append({language+'_sen': line.strip()},  ignore_index=True)
    total_pd.drop_duplicates(inplace=True)
    total_pd.to_csv(work_dir+'//'+'total_' + lang + '_sen_'+n_month+'_'+n_year+'.csv', index=False, encoding='utf-16')
    return total_pd
...
def main():
    ...
    for n_month in months[:]:
        tokenized_scrape_file(n_month,args.year,'en',args.output_dir)
        tokenized_scrape_file(n_month,args.year,'hi',args.output_dir)

# Writing all tokenized sentences to csv file
    for n_month in months[:]:
        total_sen_csv_from_tokenized_file(n_month,args.year,'en',args.output_dir)
        total_sen_csv_from_tokenized_file(n_month,args.year,'hi',args.output_dir)
