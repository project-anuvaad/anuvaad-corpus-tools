...
def get_sen_token(js, lang):
    url_en = "https://users.anuvaad.org/tokenize-sentence"
    url_hi = "https://users.anuvaad.org/tokenize-hindi-sentence"
...
def upload_document(token, filepath, fileType="file/txt"):
    ...
    data_file = [("file", data)]
    url = "https://users-auth.anuvaad.org/anuvaad-api/file-uploader/v0/upload-file"
    try:
...
def download_file(token, save_dir, file_id, prefix, extension=".txt"):
    url = "https://users-auth.anuvaad.org/download/{}".format(file_id)
...
def submit_alignment_files(token, file_id1, lang_code1, file_id2, lang_code2):
    url = "https://users-auth.anuvaad.org/anuvaad-etl/wf-manager/v1/workflow/async/initiate"
    body = {
...
def get_alignment_result(token, job_id):
    url = "https://users-auth.anuvaad.org/anuvaad-etl/wf-manager/v1/workflow/jobs/search/bulk"
    # print(url)
                ...
                if align_rsp is not None and align_rsp["jobID"] is not None:
                    ...
                    while 1:
                        ...
                        if status and rsp["jobs"][0]["status"] == "COMPLETED":
                            ...
                            print(
                                f"jobId {align_rsp['jobID']} for file ID {os.path.basename(source_filepath).split('.')[0]}, Status: {rsp['jobs'][0]['status']}"
                            )
                            rerun = False
                            break
                        elif rsp:
